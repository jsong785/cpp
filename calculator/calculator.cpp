...
parseDigit(const std::string_view equation) {
  double res = 0;
  double dec = 0;
  std::size_t i = 0;
  ...
  for (; i < equation.size(); ++i) {
    ...
    if (decimalPlace == std::string_view::npos) {
      res *= 10;
      res += (static_cast<double>(d - '0') *
              std::pow(10.0, static_cast<double>(i)));
    } else {
      dec += (static_cast<double>(d - '0') *
              std::pow(10.0, static_cast<double>(decimalPlace) - i));
    }
  }
  return {res + dec, i};
}

enum class Operation { none, add, substract, divide, multiply };

static std::pair<Operation, std::size_t>
parseOperation(const std::string_view equation) {
  const auto eq = equation[0];
  switch (eq) {
  case '+':
    return {Operation::add, 1};
  case '-':
    return {Operation::substract, 1};
  case '*':
    return {Operation::multiply, 1};
  }
...
static std::pair<double, std::size_t>
calculateParen(const std::string_view equation);
...
calculateParen(const std::string_view equation) {
  ...
  const auto len =
      static_cast<std::size_t>(std::distance(eqStart, equation.cbegin() + end));
