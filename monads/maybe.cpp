...
template <typename T, typename = void> struct transform_complex {
  using Type = T;
};

template <typename T>
struct transform_complex<
    T, std::enable_if_t<std::is_integral_v<std::decay_t<T>>>> {
...
static_assert(
    std::is_same_v<const std::string, transform_complex_t<const std::string>>);
static_assert(
    std::is_same_v<std::string &&, transform_complex_t<std::string &&>>);
static_assert(
    std::is_same_v<std::string &, transform_complex_t<std::string &>>);
static_assert(std::is_same_v<const std::string &,
                             transform_complex_t<const std::string &>>);
...
template <typename T> static constexpr auto is_maybe_v = false;

template <typename T> static constexpr auto is_maybe_v<Maybe<T>> = true;
...
template <typename T, typename Func,
          typename Ret = std::invoke_result_t<Func, T>>
...
static_assert((Just(1) | LessZero | AboveZeroHelloWorld) ==
              Just<std::string_view>("hello world"));
static_assert((Just(-1) | LessZero | AboveZeroHelloWorld) ==
              Nothing<std::string_view>());

static_assert((Just(1) | LessZero | AboveZeroHelloWorld | StringViewLength) ==
              Just(std::string_view{"hello world"}.length()));
static_assert((Just(-1) | LessZero | AboveZeroHelloWorld | StringViewLength) ==
              Nothing<std::size_t>());
